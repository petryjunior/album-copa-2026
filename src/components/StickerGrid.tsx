...
type Props = {
  ...
  /** Vista Seleções → país: cores da seleção nos cromos */
  visualTheme?: TeamVisualTheme | null
}

export function StickerGrid({ entries, qtyOf, onMarkHaveOne, onOpenEditor, visualTheme }: Props) {
  if (!entries.length) {
    return (
    ...
    <section className="grid grid-cols-[repeat(auto-fill,minmax(3.25rem,1fr))] gap-2 pb-36 sm:gap-2.5">
      {entries.map((entry) => (
        <StickerChip
          ...
          entry={entry}
          qty={qtyOf(entry.id)}
          onMarkHaveOne={() => onMarkHaveOne(entry)}
