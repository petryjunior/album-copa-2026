...
/**
 * Ordem física dentro de cada grupo: cabeça de série (pot 1) → potes 2, 3 e 4 conforme sorteio final FIFA.
 * Ajuste se sua brochura listar nomes diferentes.
 */
const TEAMS_ORDER: ReadonlyArray<{
  group: string
  code: string
  name: string
}> = [
...
export function getTeamsUnique(): ReadonlyArray<{
  group: string
  code: string
  name: string
}> {
